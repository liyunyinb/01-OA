...
public interface WmToDownGoodsMapper extends BaseMapper<WmToDownGoods> {
    ...
    List<GoodsDownInRecentVO> selectGoodsDownNumberInRecentDays(LocalDate startDate, LocalDate endDate);
    ...
    @Select("SELECT SUM(goods_qua) AS total_goods_qua, goods_name, goods_unit " +
            ...
            "LIMIT #{count}")
    List<GoodsToDownVO> selectTotalGoodsDownNumber(int count);
}
