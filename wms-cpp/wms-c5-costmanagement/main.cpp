...
void testExcel()
{
	...
	auto buffdata = excel.writeVectorToBuff(ZH_WORDS_GETTER("excel.sheet1"),data);
	const char* tsData = reinterpret_cast<const char*>(buffdata.data());
	//excel.writeVectorToFile("test.xlsx", "sheet1", data);
	FastDfsClient dfs("192.168.95.130");
	std::string result=dfs.uploadFile(tsData, buffdata.size(), "xlsx");
	if (result != " ")
	{
		std::cout << "http://192.168.95.130:8888/" << result << std::endl;
	}
