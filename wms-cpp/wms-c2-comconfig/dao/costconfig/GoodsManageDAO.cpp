...
std::list<Wm_Stt_In_GoodsDO> GoodsManageDAO::selectWithPage(const GoodsDiffPostQuery::Wrapper& query)
{
    SqlParams params;
...
// 修改数据
int GoodsManageDAO::update(const Wm_Stt_In_GoodsDO& uObj)
{
    if (stoi(uObj.getGoodsQua()) <= stoi(uObj.getSttQua()))
    {
        string sql = "UPDATE `wm_stt_in_goods` SET `stt_sta`=? WHERE `id`=?";
        return sqlSession->executeUpdate(sql, "%s%s", ZH_WORDS_GETTER("diff-post.goods.stt_sta.sta2"), uObj.getId());
    }
    return 0;
}
// 通过ID删除数据
