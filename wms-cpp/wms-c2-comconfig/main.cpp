...
void parseServerArgs(int argc, char* argv[]) {
	...
	std::string dbName = "jeewms";
	std::string dbHost = "192.168.95.128";
	int dbPort = 3306;
	int dbMax = 5;
#ifdef LINUX
	// Nacos配置参数
	std::string nacosAddr = "192.168.95.128:8848";
...
//	std::string result = dfs.uploadFile("D:\\1\\1.png");
//	if (result != "")
//	{
//		std::cout << "http://192.168.95.128:8888/" << result << std::endl;
//		// 下载文件
//		std::string dir = "file";
...
void testWxcel() {
	...
	std::string info = ("excel.content.info");
	stringstream ss;
	for (int i = 1; i <= 10; i++)
	{
	...
	// 数据写到Excel报表中
	// excel.writeVectorToFile("test.xlsx", ZH_WORDS_GETTER("excel.sheet.s1"), data);

	// 数上传到文件服务器
	auto buff = excel.writeVectorToBuff(("excel.sheet.s2"), data);
	const char* tsData = reinterpret_cast<const char*>(buff.data());
	FastDfsClient dfs("192.168.95.128");
	std::string result = dfs.uploadFile(tsData, buff.size(), "xlsx");
	if (result != "")
	{
		std::cout << "http://192.168.95.128:8888/" << result << std::endl;
	}
...
void testMq() {
	RocketClient rc("192.168.95.128:9876");
	rc.productMsgSync("wms-hello", ZH_WORDS_GETTER("excel.content.info"));
}
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		std::cerr << "Exception caught while starting server: " << e.what() << std::endl;
		return -1;
	}
	catch (...) {
		std::cerr << "Unknown exception caught while starting server." << std::endl;
		return -1;
	}
	*/
