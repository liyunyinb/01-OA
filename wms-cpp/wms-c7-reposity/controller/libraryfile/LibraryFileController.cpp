...
#include "stdafx.h"
#include <iostream>
#include "LibraryFileController.h"
#include"../wms-c7-reposity/service/libraryfile/LibraryFileService.h"
...
StringJsonVO::Wrapper LibraryFileController::execUploadLibrary(std::shared_ptr<IncomingRequest> request)
{
	// 1 初始化
	...
	/* TODO: 解析的数据具体逻辑 */
	/* 获取表单数据 */
	API_MULTIPART_PARSE_FORM_FIELD_STR(container, ZH_WORDS_GETTER("dictionary.library.id.summary"), id);
	API_MULTIPART_PARSE_FORM_FIELD_STR(container, ZH_WORDS_GETTER("dictionary.library.code.summary"), code);
	API_MULTIPART_PARSE_FORM_FIELD_STR(container, ZH_WORDS_GETTER("dictionary.library.name.summary"), name);
	/* 打印表单数据 */
...
std::shared_ptr<oatpp::web::server::api::ApiController::OutgoingResponse> LibraryFileController::execDownloadLibraryFile(const String& filename)
{
	LibraryFileService fs;
	...
	string savePath = "./public/fastdfsdeg";
