...
void parseServerArgs(int argc, char* argv[]) {
	...
	std::string dbHost = "192.168.31.239";
	int dbPort = 3306;
	int dbMax = 5;
#ifdef LINUX
	// Nacos配置参数
...
void testDfs() {
	FastDfsClient dfs("192.168.121.131");
	std::string result= dfs.uploadFile("C:\\Users\\Ambition\\Desktop\\project\\picture\\t3.jpg");
	if (result != "")
	{
		std::cout << "http://192.168.121.131:8888/" << result << std::endl; 
		//下载文件
		std::string dir = "file";
...
void testWxcel()
{
	...
	//数据上传到文件服务器
	auto buff = excel.writeVectorToBuff(ZH_WORDS_GETTER("excel.sheet.s2"), data);
	const char* tsData = reinterpret_cast<const char*>(buff.data());
	FastDfsClient dfs("192.168.121.131");
	std::string result = dfs.uploadFile(tsData, data.size(), "xlsx");
	if (result != "")
	{
		std::cout << "http://192.168.121.131:8888/" << result << std::endl;
	}
